...
def _llm_editor(or_key, story, sources):
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            'HTTP-Referer': 'https://tsquirrel.pixagreat.com',
            'X-Title': 'TSquirrel Quality Gate',
        },
        method='POST',
    )
...
def run(limit=20, dry_run=False):
    ...
    save_state(state)
    print(f"editorial_step done | edited={len(edited)} skipped={len(skipped)} dry_run={str(bool(dry_run)).lower()} model={MODEL}")
