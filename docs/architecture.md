...
| 18 | Premium translation workflow: language selection + deep analysis + private embargo to public release | ⬜ todo | Productize paid flow: language output selector, deep analysis mode, longer privacy windows by tier, "keep private forever" upsell, "publish now" generosity toggle, and auto-publish after N days only when source/license permits. |
